# tests/rhosocial/activerecord_test/feature/backend/sqlite/extensions/test_fts5_async.py
"""Async twin of test_fts5.py: expression construction stays sync; backend scenarios run on AsyncSQLiteBackend."""
import pytest
...
# =============================================================================
# Part 1: Expression Construction Tests — FTS5 support & capability (sync-only)
# =============================================================================

class TestFTS5Support:
    """FTS5 support detection — covers all capability check methods (pure dialect booleans)."""
...
class TestFTS5Tokenizers:
    """FTS5 tokenizer support — version-gated (pure dialect)."""
...
class TestSQLiteFTS5CreateVirtualTableConstruction:
    """100% coverage of SQLiteFTS5CreateVirtualTable construction and format_* (pure expressions)."""
...
class TestSQLiteMatchPredicateConstruction:
    """100% coverage of SQLiteMatchPredicate construction and format_* (pure expressions)."""
...
class TestSQLiteFTS5RankExpressionConstruction:
    """100% coverage of SQLiteFTS5RankExpression construction and format_* (pure expressions)."""
...
class TestSQLiteFTS5HighlightExpressionConstruction:
    """100% coverage of SQLiteFTS5HighlightExpression construction and format_* (pure expressions)."""
...
class TestSQLiteFTS5SnippetExpressionConstruction:
    """100% coverage of SQLiteFTS5SnippetExpression construction and format_* (pure expressions)."""
...
class TestMatchPredicateConstruction:
    """SQLiteMatchPredicate expression class coverage (pure expressions)."""
...
class TestFormatMatchPredicate:
    """dialect.format_match_predicate method coverage (pure dialect)."""
...
# =============================================================================
# Part 2: Real-World Scenario Tests — FTS5 with AsyncSQLiteBackend
# =============================================================================

class TestFTS5Scenario:
    """Real FTS5 scenarios executed against an in-memory AsyncSQLiteBackend."""
    ...
    @pytest.mark.asyncio
    async def test_create_and_search(self, backend):
        ...
        await backend.execute("INSERT INTO articles VALUES (?, ?)", ("A", "hello world"), options=insert)
        await backend.execute("INSERT INTO articles VALUES (?, ?)", ("B", "hello python"), options=insert)
        await backend.execute("INSERT INTO articles VALUES (?, ?)", ("C", "sqlite database"), options=insert)

        rows = await backend.fetch_all("SELECT title FROM articles WHERE articles MATCH ?", ("hello",))
        assert len(rows) == 2

    @pytest.mark.asyncio
    async def test_bm25_ranking(self, backend):
        dialect = backend.dialect
        if not dialect.supports_fts5():
            pytest.skip("FTS5 not available")
        ...
        insert = ExecutionOptions(stmt_type=StatementType.INSERT)
        await backend.execute("INSERT INTO docs VALUES (?, ?)", ("A", "python python python"), options=insert)
        await backend.execute("INSERT INTO docs VALUES (?, ?)", ("B", "python"), options=insert)
    ...
    @pytest.mark.asyncio
    async def test_porter_tokenizer(self, backend):
        dialect = backend.dialect
        if not dialect.supports_fts5():
            pytest.skip("FTS5 not available")
        ...
        insert = ExecutionOptions(stmt_type=StatementType.INSERT)
        await backend.execute("INSERT INTO posts VALUES (?)", ("running jumps",), options=insert)

        rows = await backend.fetch_all("SELECT content FROM posts WHERE posts MATCH ?", ("run",))
        assert len(rows) == 1

    @pytest.mark.asyncio
    async def test_unicode61_diacritics(self, backend):
        dialect = backend.dialect
        if not dialect.supports_fts5():
            pytest.skip("FTS5 not available")
        ...
        insert = ExecutionOptions(stmt_type=StatementType.INSERT)
        await backend.execute("INSERT INTO texts VALUES (?)", ("café",), options=insert)

        rows = await backend.fetch_all("SELECT content FROM texts WHERE texts MATCH ?", ("cafe",))
        assert len(rows) == 1

    @pytest.mark.asyncio
    async def test_prefix_indexing(self, backend):
        dialect = backend.dialect
        if not dialect.supports_fts5():
            pytest.skip("FTS5 not available")
        ...
        insert = ExecutionOptions(stmt_type=StatementType.INSERT)
        await backend.execute("INSERT INTO prefixes VALUES (?)", ("hello world",), options=insert)

        rows = await backend.fetch_all("SELECT content FROM prefixes WHERE prefixes MATCH ?", ("he*",))
        assert len(rows) == 1

    @pytest.mark.asyncio
    async def test_drop_virtual_table(self, backend):
        dialect = backend.dialect
        if not dialect.supports_fts5():
            pytest.skip("FTS5 not available")
        ...
        rows = await backend.fetch_all("SELECT name FROM sqlite_master WHERE name='tmp'")
        assert len(rows) == 1

        drop_sql, _ = dialect.format_drop_virtual_table("tmp")
        await backend.execute(drop_sql, options=ExecutionOptions(stmt_type=StatementType.DDL))
