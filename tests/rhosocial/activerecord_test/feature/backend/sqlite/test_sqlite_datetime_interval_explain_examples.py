# tests/rhosocial/activerecord_test/feature/backend/sqlite/test_sqlite_datetime_interval_explain_examples.py
"""SQLite EXPLAIN examples for datetime interval expressions and indexes."""
...
def temporal_backend():
    ...
    for statement in _SETUP_STATEMENTS:
        backend.execute(statement, options=ExecutionOptions(stmt_type=StatementType.DDL))
    yield backend
    backend.disconnect()
...
def test_created_at_range_uses_datetime_index(temporal_backend, query_plan_options):
    ...
    result = temporal_backend.explain(query, query_plan_options)
...
def test_started_at_range_uses_composite_datetime_index(temporal_backend, query_plan_options):
    dialect = temporal_backend.dialect
    query = QueryExpression(
        dialect,
...
def test_datetime_interval_expressions_work_with_indexed_filter(
    ...
    explain_result = temporal_backend.explain(query, query_plan_options)
    ...
    query_result = temporal_backend.execute(
        *query.to_sql(),
