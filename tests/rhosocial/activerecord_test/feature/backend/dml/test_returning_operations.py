# tests/rhosocial/activerecord_test/feature/backend/dml/test_returning_operations.py
"""
...
def test_delete_with_returning_columns(returning_backend):
    ...
    insert_options = InsertOptions(
        table="test_users",
        data={"name": "ToDelete User", "email": "to_delete@example.com", "age": 35},
        returning_columns=["user_id"],
    )
    insert_result = backend.insert(insert_options)
    user_id = insert_result.data[0]["user_id"]
    ...
    check_result = backend.fetch_one("SELECT * FROM test_users WHERE user_id = ?", (user_id,))
...
def test_operations_without_returning_columns(returning_backend):
    ...
    insert_options = InsertOptions(
        table="test_users",
        data={"name": "No Return User", "email": "no_return@example.com", "age": 40},
...
def test_multiple_returning_columns(returning_backend):
    ...
    insert_options = InsertOptions(
        table="test_users",
        data={"name": "Multi Return User", "email": "multi_return@example.com", "age": 33, "is_active": 1},
