# tests/rhosocial/activerecord_test/feature/backend/ddl/test_composite_pk_async.py
"""Async twin of test_composite_pk.py using AsyncSQLiteBackend."""
import pytest
...
async def composite_pk_backend(temp_db_path):
    """Create a file-based AsyncSQLiteBackend seeded with a composite-PK table."""
...
class TestAsyncCompositePKDDL:
    @pytest.mark.asyncio
    async def test_create_table(self, composite_pk_backend: AsyncSQLiteBackend):
        result = await composite_pk_backend.execute(
    ...
    @pytest.mark.asyncio
    async def test_insert_unique_constraint(self, composite_pk_backend: AsyncSQLiteBackend):
        with pytest.raises(IntegrityError):
            await composite_pk_backend.execute("INSERT INTO order_items VALUES (1, 101, 99)")


class TestAsyncCompositePKDML:
    @pytest.mark.asyncio
    async def test_update_by_composite_pk(self, composite_pk_backend: AsyncSQLiteBackend):
        await composite_pk_backend.execute(
    ...
    @pytest.mark.asyncio
    async def test_delete_by_composite_pk(self, composite_pk_backend: AsyncSQLiteBackend):
        await composite_pk_backend.execute("DELETE FROM order_items WHERE order_id = 1 AND product_id = 101")
        result = await composite_pk_backend.execute(
...
class TestAsyncCompositePKExplain:
    @pytest.mark.asyncio
    async def test_explain_pk_lookup(self, composite_pk_backend: AsyncSQLiteBackend):
        result = await composite_pk_backend.execute(
    ...
    @pytest.mark.asyncio
    async def test_explain_prefix_only(self, composite_pk_backend: AsyncSQLiteBackend):
        result = await composite_pk_backend.execute(
...
class TestAsyncCompositePKReturning:
    @pytest.mark.asyncio
    async def test_returning_insert(self, composite_pk_backend: AsyncSQLiteBackend):
