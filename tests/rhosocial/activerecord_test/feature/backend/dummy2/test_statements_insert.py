...
class TestInsertStatements:
    ...
    def test_insert_data_source_dialect_property(self, dummy_dialect: DummyDialect):
        ...
        source = ValuesSource(dummy_dialect, values_list=[[Literal(dummy_dialect, "John Doe")]])
    ...
    @pytest.mark.parametrize(
        "conflict_options, error_message",
        [
            pytest.param(
    ...
    def test_on_conflict_clause_init_errors(self, dummy_dialect: DummyDialect, conflict_options, error_message):
        ...
        with pytest.raises(ValueError, match=error_message):
            OnConflictClause(dummy_dialect, conflict_target=["id"], **conflict_options)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                ("event_data",),
                id="single_row_insert_columns_none",
            ),
            ...
            pytest.param(
                ...
                (100.50,),
                id="single_row_insert_returning",
            ),
            ...
            pytest.param(
                ...
                (10,),
                id="single_row_insert_on_conflict_do_nothing",
            ),
            ...
            pytest.param(
                ...
                None,
                OnConflictClause(
                    None, conflict_target=["key"], update_assignments={"value": Column(None, "value", "excluded")}
                ),
                1,
                'INSERT INTO "metrics" ("key", "value") VALUES (?, ?) ON CONFLICT ("key") DO UPDATE SET "value" = "excluded"."value"',  # noqa: E501
                ("cpu_usage", 85.5),
                id="single_row_insert_on_conflict_do_update",
            ),
            # --- Multi row inserts ---
            ...
            pytest.param(
                ...
                (1, 5, 2, 10),
                id="multi_row_insert_on_conflict_do_update_where",
            ),
        ],
    )
    def test_insert_with_values_source_combinations(
        self,
        ...
        def set_dialect_recursive(expr, dialect):
            ...
            if hasattr(expr, "value") and isinstance(
                expr.value, bases.BaseExpression
            ):  # For Literal, if value is an expression
        ...
        values_data = []
        if num_rows == 1:
            if columns_param == ["name", "email"]:
                values_data = [["John Doe", "john@example.com"]]
            elif columns_param == ["product_name"]:
                values_data = [["Laptop"]]
            elif columns_param == ["amount"]:
                values_data = [[100.50]]
            elif columns_param == ["item_id"]:
                values_data = [[10]]
            elif columns_param == ["key", "value"]:
                values_data = [["cpu_usage", 85.5]]
            elif columns_param is None:
                values_data = [["event_data"]]  # For columns=None case
        elif num_rows == 2:
            if columns_param == ["name", "email"]:
                values_data = [["Jane Doe", "jane@example.com"], ["Peter Pan", "peter@example.com"]]
            elif columns_param == ["txn_id", "status"]:
                values_data = [["TXN001", "success"], ["TXN002", "pending"]]
            elif columns_param == ["p_id", "qty"]:
                values_data = [[1, 5], [2, 10]]
        ...
        if on_conflict_param:
            set_dialect_recursive(
                on_conflict_param, dummy_dialect
        ...
        if columns_param:
            ...
            assert re.search(match_pattern, sql) is not None, (
                f"Column definition '{expected_column_str_with_parens}' not found or incorrectly placed after table '{table_sql_part}' in SQL: {sql}"  # noqa: E501
            )
            ...
            assert (
                re.search(match_pattern_default_values, sql) is not None
                or re.search(match_pattern_values, sql) is not None
    ...
    def test_insert_on_conflict_do_update(self, dummy_dialect: DummyDialect):
        ...
        source = ValuesSource(
            dummy_dialect, values_list=[[Literal(dummy_dialect, 1), Literal(dummy_dialect, "Updated Name")]]
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ["col1"], [[""]], 'INSERT INTO "test_table" ("col1") VALUES (?)', ("",), id="single_empty_string"
            ),
            pytest.param(
                ["col1", "col2"],
                [["", ""]],
    ...
    def test_insert_empty_string_values(
        self, dummy_dialect: DummyDialect, columns_param, values_data, expected_sql, expected_params
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ["col1"], [[None]], 'INSERT INTO "test_table" ("col1") VALUES (?)', (None,), id="single_null_value"
            ),
            pytest.param(
                ["col1", "col2"],
                [[None, "test"]],
    ...
    def test_on_conflict_multiple_columns_target(self, dummy_dialect: DummyDialect):
        ...
        sql, params = insert_expr.to_sql()
        expected_sql = (
            'INSERT INTO "users" ("id", "email", "name") VALUES (?, ?, ?) ON CONFLICT ("col1", "col2") DO NOTHING'
        )
        assert sql == expected_sql
    ...
    def test_on_conflict_with_expression_target(self, dummy_dialect: DummyDialect):
        ...
        source = ValuesSource(
            dummy_dialect, values_list=[[Literal(dummy_dialect, 1), Literal(dummy_dialect, "TEST@EXAMPLE.COM")]]
        )
        on_conflict = OnConflictClause(
            dummy_dialect,
            conflict_target=[
                RawSQLExpression(dummy_dialect, 'lower("email")')
            ],  # Using RawSQLExpression for the target
            do_nothing=True,
        )
        insert_expr = InsertExpression(
        ...
        )  # Dialect is expected to format raw SQL correctly
        assert sql == expected_sql
    ...
    def test_on_conflict_update_multiple_columns_complex(self, dummy_dialect: DummyDialect):
        ...
        source = ValuesSource(
            dummy_dialect,
            values_list=[[Literal(dummy_dialect, 1), Literal(dummy_dialect, "new_name"), Literal(dummy_dialect, 5)]],
        )
        on_conflict = OnConflictClause(
            dummy_dialect,
            conflict_target=["id"],
            update_assignments={
                ...
                "score": RawSQLExpression(
                    dummy_dialect,
                    f"{dummy_dialect.format_identifier('users')}.{dummy_dialect.format_identifier('score')} + {dummy_dialect.format_identifier('excluded')}.{dummy_dialect.format_identifier('new_score')}",  # noqa: E501
                ),
            },
        )
        insert_expr = InsertExpression(
        ...
        expected_sql = (
            'INSERT INTO "users" ("id", "name", "new_score") VALUES (?, ?, ?) '
            'ON CONFLICT ("id") DO UPDATE SET "name" = "excluded"."name", "score" = "users"."score" + "excluded"."new_score"'  # noqa: E501
        )
        assert sql == expected_sql
    ...
    def test_on_conflict_update_where_with_complex_predicate(self, dummy_dialect: DummyDialect):
        ...
        source = ValuesSource(
            dummy_dialect,
            values_list=[[Literal(dummy_dialect, 1), Literal(dummy_dialect, "inactive"), Literal(dummy_dialect, 10)]],
        )
        on_conflict = OnConflictClause(
            dummy_dialect,
            conflict_target=["id"],
