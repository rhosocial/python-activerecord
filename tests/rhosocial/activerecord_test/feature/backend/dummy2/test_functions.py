...
from rhosocial.activerecord.backend.expression import (
    ...
    json_array_elements,
    json_objectagg,
    json_arrayagg,  # Added these new JSON aggregation functions
    array_agg,
    unnest,
    array_length,
    cast,
...
class TestJsonFunctionFactoriesExtended:
    ...
    def test_format_function_call_with_filter_clause_on_non_supporting_dialect_raises_error(
        self, dummy_dialect: DummyDialect
    ):
        """Tests that format_function_call raises UnsupportedFeatureError when filter clause is used on non-supporting dialect."""  # noqa: E501
