...
class TestLateralExpression:
    ...
    def test_lateral_subquery_basic(self, dummy_dialect: DummyDialect):
        ...
        lateral_query = Subquery(
            dummy_dialect, "SELECT user_id, order_date FROM orders WHERE user_id = users.id LIMIT ?", (5,)
        )

        lateral_expr = LateralExpression(dummy_dialect, expression=lateral_query, alias="user_orders")
    ...
    def test_lateral_table_function(self, dummy_dialect: DummyDialect):
        ...
        table_func = TableFunctionExpression(
            dummy_dialect, "UNNEST", Literal(dummy_dialect, [1, 2, 3]), alias="unnested", column_names=["value"]
        )

        lateral_expr = LateralExpression(dummy_dialect, expression=table_func, alias="lateral_unnest")
    ...
    def test_lateral_with_cross_join_type(self, dummy_dialect: DummyDialect):
        ...
        lateral_query = Subquery(dummy_dialect, "SELECT * FROM related_table WHERE parent_id = main_table.id", ())

        lateral_expr = LateralExpression(
            dummy_dialect, expression=lateral_query, alias="related_data", join_type="CROSS"
        )
    ...
    def test_lateral_with_inner_join_type(self, dummy_dialect: DummyDialect):
        ...
        lateral_query = Subquery(
            dummy_dialect, "SELECT item_id, quantity FROM order_items WHERE order_id = orders.id", ()
        )

        lateral_expr = LateralExpression(dummy_dialect, expression=lateral_query, alias="items", join_type="INNER")
    ...
    def test_lateral_used_in_query_from_clause(self, dummy_dialect: DummyDialect):
        ...
        lateral_query = Subquery(
            dummy_dialect, "SELECT COUNT(*) as order_count FROM orders WHERE user_id = users.id", ()
        )

        lateral_expr = LateralExpression(dummy_dialect, expression=lateral_query, alias="user_stats")

        # Create query using just the lateral expression as FROM
        query = QueryExpression(
            dummy_dialect,
            select=[Column(dummy_dialect, "order_count", table="user_stats")],
    ...
    def test_lateral_with_values_expression(self, dummy_dialect: DummyDialect):
        ...
        values_expr = ValuesExpression(
            dummy_dialect, values=[(1, "test"), (2, "data")], alias="value_table", column_names=["id", "name"]
        )

        lateral_expr = LateralExpression(dummy_dialect, expression=values_expr, alias="lateral_values")
    ...
    def test_lateral_expression_complex_subquery(self, dummy_dialect: DummyDialect):
        ...
        lateral_expr = LateralExpression(dummy_dialect, expression=complex_query, alias="aggregated_data")
    ...
    def test_lateral_with_different_join_types(self, dummy_dialect: DummyDialect):
        ...
        for join_type in join_types:
            lateral_expr = LateralExpression(
                dummy_dialect, expression=lateral_query, alias=f"lateral_{join_type.lower()}", join_type=join_type
            )
    ...
    def test_lateral_expression_parameters_handling(self, dummy_dialect: DummyDialect):
        ...
        lateral_query = Subquery(dummy_dialect, "SELECT * FROM table WHERE col1 = ? AND col2 = ?", ("value1", "value2"))

        lateral_expr = LateralExpression(dummy_dialect, expression=lateral_query, alias="param_lateral")
    ...
    def test_lateral_with_alias_formatting(self, dummy_dialect: DummyDialect):
        ...
        lateral_expr = LateralExpression(dummy_dialect, expression=lateral_query, alias="formatted_alias")
    ...
    def test_lateral_without_alias(self, dummy_dialect: DummyDialect):
        ...
        lateral_expr = LateralExpression(dummy_dialect, expression=lateral_query)
