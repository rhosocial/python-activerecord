# tests/rhosocial/activerecord_test/feature/backend/dummy2/test_expressions_predicates.py
import pytest
...
class TestPredicateExpressions:
    ...
    def test_comparison_predicate(
        self, dummy_dialect: DummyDialect, left_data, operator, right_data, expected_sql, expected_params
    ):
    ...
    @pytest.mark.parametrize(
        "expr_data, op, pattern_data, expected_sql, expected_params",
        [
    ...
    def test_like_predicate(
        self, dummy_dialect: DummyDialect, expr_data, op, pattern_data, expected_sql, expected_params
    ):
    ...
    def test_in_predicate_with_list(self, dummy_dialect: DummyDialect):
        ...
        pred = InPredicate(
            dummy_dialect, Column(dummy_dialect, "status"), Literal(dummy_dialect, ["active", "pending"])
        )
        sql, params = pred.to_sql()
    ...
    @pytest.mark.parametrize(
        "expr_data, lower_data, upper_data, expected_sql, expected_params",
        [
            ((Column, ("quantity",)), (Literal, (10,)), (Literal, (20,)), '"quantity" BETWEEN ? AND ?', (10, 20)),
            (
                (RawSQLExpression, ("order_date",)),
                (Literal, ("2023-01-01",)),
                (Literal, ("2023-12-31",)),
    ...
    def test_between_predicate(
        self, dummy_dialect: DummyDialect, expr_data, lower_data, upper_data, expected_sql, expected_params
    ):
    ...
    @pytest.mark.parametrize(
        "expr_data, is_not, expected_sql, expected_params",
        [
    ...
    def test_complex_logical_predicate(self, dummy_dialect: DummyDialect):
        ...
        status_pred = ComparisonPredicate(
            dummy_dialect, "=", Column(dummy_dialect, "status"), Literal(dummy_dialect, "active")
        )
        name_like_pred = LikePredicate(
            dummy_dialect, "LIKE", Column(dummy_dialect, "name"), Literal(dummy_dialect, "J%")
        )
    ...
    @pytest.mark.parametrize(
        "expr_data, value, is_not, expected_sql, expected_params",
        [
    ...
    def test_is_boolean_predicate(
        self, dummy_dialect: DummyDialect, expr_data, value, is_not, expected_sql, expected_params
    ):
