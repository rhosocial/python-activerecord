# tests/rhosocial/activerecord_test/feature/backend/dialect/test_sqlite_dialect_version_support.py
"""
...
class TestSQLiteDialectVersionSupport:
    ...
    def test_unsupported_features_always_false(self):
        ...
        assert not dialect.supports_graph_match()
        assert not dialect.supports_merge_statement()
        assert not dialect.supports_temporal_tables()
        assert not dialect.supports_qualify_clause()
        assert not dialect.supports_ordered_set_aggregation()
