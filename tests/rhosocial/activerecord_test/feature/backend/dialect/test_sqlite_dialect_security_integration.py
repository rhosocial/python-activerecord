# tests/rhosocial/activerecord_test/feature/backend/dialect/test_sqlite_dialect_security_integration.py
"""
...
class TestSQLiteDialectSecurityIntegration:
    ...
    def test_table_with_special_chars(self, backend):
        ...
        backend.execute(
            "DROP TABLE IF EXISTS test_security_chars", options=ExecutionOptions(stmt_type=StatementType.DDL)
        )

    def test_default_string_with_single_quote_insert_and_retrieve(self, backend, test_table_with_special_chars):
        ...
        # Retrieve and verify
        result = backend.fetch_one("SELECT name FROM test_security_chars WHERE id = 1")
    ...
    def test_table_for_data_type(self, backend):
        ...
        backend.execute(
            "DROP TABLE IF EXISTS test_data_type_security", options=ExecutionOptions(stmt_type=StatementType.DDL)
        )
    ...
    def test_valid_data_type_works(self, backend, test_table_for_data_type):
        ...
        result = backend.fetch_one(f"SELECT name FROM sqlite_master WHERE name = '{test_table_for_data_type}'")
...
class TestSQLiteDefaultConstraintSecurityIntegration:
    ...
    def test_default_with_single_quote_retrieved_correctly(self, backend):
        ...
        result = backend.fetch_one("SELECT sql FROM sqlite_master WHERE name = 'test_default_quote'")
        assert result is not None
        assert "it''s a test" in result["sql"]

        backend.execute(
            "DROP TABLE IF EXISTS test_default_quote", options=ExecutionOptions(stmt_type=StatementType.DDL)
        )
...
class TestSQLiteStorageOptionsSecurityIntegration:
    ...
    def test_storage_options_work(self, backend):
        ...
        backend.execute("DROP TABLE IF EXISTS test_storage_opts", options=ExecutionOptions(stmt_type=StatementType.DDL))
        backend.execute(
            """
        ...
        result = backend.fetch_one("SELECT name FROM sqlite_master WHERE name = 'test_storage_opts'")
        assert result is not None
