...
- B1/B2  typed ``DDLPlan`` creation/teardown plans.
"""
...
from rhosocial.activerecord.base import UseColumnAttributes, UseConstraint, UseIndex, UseSqlType
...
from rhosocial.activerecord.base.ddl.params import StatementContractError, StatementParamSchema
from rhosocial.activerecord.base.ddl.plans import DDLPlan
...
def test_composite_pk_member_gets_not_null_and_table_pk(backend):
    sql, _ = CompositeWithIndex.create_table().to_sql()
...
def test_identity_attribute_is_collected_and_rendered(backend):
    ...
    sql, _ = WithIdentity.create_table().to_sql()
...
def test_identity_attribute_parameters_are_rendered(backend):
    sql, _ = WithAttributes.create_table().to_sql()
...
def test_foreign_attribute_is_skipped(backend):
    ...
    assert WithForeignAttribute.create_table().columns[1].attributes == []
...
def test_unknown_generic_attribute_raises(backend):
    ...
    with pytest.raises(Exception, match="column_attributes"):
        WithUnknownAttribute.create_table()
...
def test_field_level_and_table_level_indexes_are_merged_and_deduped(backend):
    ...
    indexes = CompositeWithIndex.create_indexes()
...
def test_overriding_table_indexes_leaves_field_level_intact(backend):
    ...
    indexes = WithTableIndexesOverride.create_indexes()
...
def test_table_constraints_constant_is_read(backend):
    ...
    sql, _ = WithTableConstraints.create_table().to_sql()
...
def test_single_value_all_inapplicable_raises(backend):
    ...
    with pytest.raises(Exception, match="table_partition"):
        WithForeignPartition.create_table()
...
def test_additive_foreign_candidates_are_skipped(backend):
    ...
    names = [expr.index_name for expr in MixedIndexes.create_indexes()]
...
def test_explicit_null_is_kept_for_non_pk(backend):
    sql, _ = WithNullable.create_table().to_sql()
...
def test_explicit_null_on_pk_member_is_discarded(backend):
    ...
    sql, _ = NullablePk.create_table().to_sql()
...
def test_statement_candidates_default_to_generic(backend):
    expression = WithNullable.create_table()
...
def test_statement_candidates_model_override_selects_most_fitting(backend):
    ...
    try:
        expression = OverrideStatementCandidates.create_table()
        assert type(expression) is CreateTableExpression
    finally:
...
def test_statement_candidates_model_override_all_inapplicable_raises(backend):
    ...
    try:
        with pytest.raises(Exception, match="create_table"):
            OverrideStatementCandidates.create_table()
    finally:
        OverrideStatementCandidates.__create_table_statement__ = None
...
def test_statement_candidates_dialect_preferred_class_is_used():
    ...
    expression = PreferredModel.create_table()
    assert type(expression) is CreateTableExpression
...
# ---------------------------------------------------------------------------
# B1/B2 typed DDLPlan + B9 column_name contract
# ---------------------------------------------------------------------------


def test_creation_plan_and_teardown_plan_are_typed(backend):
    creation = WithNullable.creation_plan()
    teardown = WithNullable.teardown_plan()
    assert isinstance(creation, DDLPlan)
    assert isinstance(teardown, DDLPlan)
    assert isinstance(creation[0], CreateTableExpression)
    assert len(creation) == 1
    assert isinstance(teardown[0], BaseExpression)
    rendered = creation.to_sql()
    assert len(rendered) == 1
    assert rendered[0][0].startswith("CREATE TABLE")
...
def test_no_format_statement_class_is_not_renderable():
    ...
    from rhosocial.activerecord.base.ddl.deriver import TableDDLDeriver
...
def test_inline_indexes_override_true_raises_on_sqlite(backend):
    expression = WithTableIndexesOverride.create_table(inline_indexes=True)
...
def test_identity_attribute_with_parameters_survives_spec_roundtrip(backend):
    original = WithIdentity.create_table()
    spec = WithIdentity.create_table_spec()
    rebuilt = WithIdentity.create_table_from_spec(spec)
