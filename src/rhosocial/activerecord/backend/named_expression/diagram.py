...
class ProcedureDiagram:
    ...
    def _end_label(self) -> str:
        if self.instance_trace is None:
            return "End"
        total = sum(e.elapsed_ms for e in self.instance_trace if e.elapsed_ms is not None)
    ...
    def _render_flowchart(self) -> str:
        ...
        if self.is_static:
            lines.append("%% [Static diagram — generated by dry-run. Conditional branches may be incomplete.]")
        ...
        for entry in self.static_trace:
            if entry.kind == StepKind.SINGLE:
                ...
                if self.is_static:
                    ...
                    styles.append(f"    style {nid} {_STATUS_COLOR.get(ex.status or 'ok', _NEUTRAL_COLOR)}")
                else:
        ...
        for entry in self.static_trace:
            if entry.kind == StepKind.SINGLE:
                ...
                for i, sub in enumerate(entry.sub_steps):
                    ...
                    hint = f"  %% max_concurrency={entry.max_concurrency}" if i == 0 and entry.max_concurrency else ""
                    lines.append(f"    {keyword} {sub.qualified_name or f'step_{sub.index}'}{hint}")
    ...
    def _seq_single(
        ...
        if ex is not None:
            if ex.status == "error":
                ...
                lines.append(f"{pad}note right of Runner: ✗ {ex.error or 'error'}{elapsed}")
            else:
