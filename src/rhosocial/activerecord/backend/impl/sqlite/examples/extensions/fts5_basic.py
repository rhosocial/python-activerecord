...
expr = FTS5CreateVirtualTable(dialect, table_name="ascii_docs", columns=["title", "body"], tokenizer="ascii")
sql, _ = expr.to_sql()
...
for title, body in articles_data:
    backend.execute("INSERT INTO articles(title, body) VALUES (?, ?)", (title, body), options=dml_opts)
...
for (content,) in posts_data:
    backend.execute("INSERT INTO posts(content) VALUES (?)", (content,), options=dml_opts)
...
for (content,) in texts_data:
    backend.execute("INSERT INTO texts(content) VALUES (?)", (content,), options=dml_opts)
...
for title, body in ascii_data:
    backend.execute("INSERT INTO ascii_docs(title, body) VALUES (?, ?)", (title, body), options=dml_opts)
...
python_count = backend.fetch_all("SELECT count(*) as cnt FROM articles WHERE articles MATCH ?", ("python",))[0]["cnt"]
...
print("\n  --- 4a. Default BM25 ranking ---")
sql = "SELECT title, bm25(articles) as rank FROM articles WHERE articles MATCH ? ORDER BY rank"
...
sql = "SELECT title, bm25(articles, 10.0, 1.0) as rank FROM articles WHERE articles MATCH ? ORDER BY rank"
r = backend.fetch_all(sql, ("python",))
print_results("python, weighted title=10x body=1x", r, ["title", "rank"])

# 4c. Custom BM25 parameters (k1, b)
print("\n  --- 4c. Custom BM25 parameters ---")
sql = "SELECT title, bm25(articles, 'k1', 1.5, 'b', 0.75) as rank FROM articles WHERE articles MATCH ? ORDER BY rank"
r = backend.fetch_all(sql, ("python",))
...
hl_expr = FTS5HighlightExpression(dialect, table_name="articles", column="title")
...
sql = "SELECT highlight(articles, body, '[[', ']]') as highlighted, title FROM articles WHERE articles MATCH ?"
r = backend.fetch_all(sql, ("database",))
for row in r:
...
sql = "SELECT snippet(articles, body, '<b>', '</b>', '...', 10) as snip, title FROM articles WHERE articles MATCH ?"
...
sql = "SELECT snippet(articles, body, '[', ']', '..', 3) as snip, title FROM articles WHERE articles MATCH ?"
r = backend.fetch_all(sql, ("python",))
for row in r:
    print(f"    [{row['title']}] {row['snip']}")
...
print("=" * 60)
sql = "SELECT title, rank FROM articles WHERE articles MATCH ? ORDER BY rank"
r = backend.fetch_all(sql, ("python",))
...
r = backend.fetch_all("SELECT rowid, title, body FROM ascii_docs WHERE ascii_docs MATCH ?", ("hello",))
print(f"    MATCH 'hello': {len(r)} result(s) (expected 1 — 'English Only')")

r = backend.fetch_all("SELECT rowid, title, body FROM ascii_docs WHERE ascii_docs MATCH ?", ("naïve",))
print(f"    MATCH 'naïve': {len(r)} result(s) (expected 0 — ASCII tokenizer splits at non-ASCII chars)")

r = backend.fetch_all("SELECT rowid, title, body FROM ascii_docs WHERE ascii_docs MATCH ?", ("100",))
...
for query, desc in test_cases:
    r = backend.fetch_all("SELECT rowid, content FROM posts WHERE posts MATCH ?", (query,))
...
for query, desc in test_cases:
    r = backend.fetch_all("SELECT rowid, content FROM texts WHERE texts MATCH ?", (query,))
    match_count = len(r)
...
if "trigram" in dialect.get_supported_fts5_tokenizers():
    expr = FTS5CreateVirtualTable(dialect, table_name="substrings", columns=["content"], tokenizer="trigram")
    sql, _ = expr.to_sql()
    backend.execute(sql, options=ddl_opts)
    ...
    for (content,) in trigram_data:
        backend.execute("INSERT INTO substrings(content) VALUES (?)", (content,), options=dml_opts)
    ...
    for query in ["wor", "ogr", "sql", "tho"]:
        r = backend.fetch_all("SELECT rowid, content FROM substrings WHERE substrings MATCH ?", (query,))
...
print(
    f"\n  Remaining tables after cleanup: {len(remaining)} (all clean)"
    if len(remaining) == 0
