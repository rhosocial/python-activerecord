...
from rhosocial.activerecord.backend.impl.sqlite.pragma import get_pragma_info
...
class PragmaMixin:
    ...
    def _pragma_sql(
        ...
        Raises:
            ValueError: If the PRAGMA name is not in the known whitelist.
        """
        ...
        if argument is not None:
            return (f"PRAGMA {prefix}{info.name}({argument})", ())
        return (f"PRAGMA {prefix}{info.name}", ())

    @staticmethod
    def _set_pragma_sql(name: str, value: Any, schema: Optional[str] = None) -> Tuple[str, tuple]:
        """Build a ``PRAGMA name = value`` assignment statement.

        Validates the PRAGMA name against the known whitelist, rejects
        read-only PRAGMAs, and enforces ``allowed_values`` before building
        the SQL, mirroring ``SQLitePragmaMixin.set_pragma_sql``.
        ...
        Raises:
            ValueError: If the PRAGMA is unknown, read-only, or the value
                        is not in its allowed values.
        """
        info = get_pragma_info(name)
        if info is None:
            raise ValueError(f"Unknown PRAGMA: {name}")
        ...
        prefix = f"{schema}." if schema else ""
        return (f"PRAGMA {prefix}{info.name} = {value}", ())
...
class SyncPragmaIntrospector(PragmaMixin):
    ...
    def table_info(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA table_info(table)`` rows."""
        return self.execute("table_info", self._backend.dialect.format_identifier(table), schema)

    def table_xinfo(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA table_xinfo(table)`` rows (SQLite 3.26+).

        Includes hidden columns generated by virtual tables.
        """
        return self.execute("table_xinfo", self._backend.dialect.format_identifier(table), schema)

    def index_list(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA index_list(table)`` rows."""
        return self.execute("index_list", self._backend.dialect.format_identifier(table), schema)

    def index_info(self, index: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA index_info(index)`` rows."""
        return self.execute("index_info", self._backend.dialect.format_identifier(index), schema)

    def index_xinfo(self, index: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA index_xinfo(index)`` rows (SQLite 3.9+)."""
        return self.execute("index_xinfo", self._backend.dialect.format_identifier(index), schema)

    def foreign_key_list(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA foreign_key_list(table)`` rows."""
        return self.execute("foreign_key_list", self._backend.dialect.format_identifier(table), schema)
    ...
    def integrity_check(self, schema: Optional[str] = None) -> List[str]:
    ...
    def foreign_key_check(
        ...
        """
        arg = self._backend.dialect.format_identifier(table) if table else None
        return self.execute("foreign_key_check", arg, schema)

    def journal_mode(self, schema: Optional[str] = None) -> Optional[str]:
...
class AsyncPragmaIntrospector(PragmaMixin):
    ...
    async def table_info(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA table_info(table)`` rows."""
        return await self.execute("table_info", self._backend.dialect.format_identifier(table), schema)

    async def table_xinfo(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA table_xinfo(table)`` rows (SQLite 3.26+).

        Includes hidden columns generated by virtual tables.
        """
        return await self.execute("table_xinfo", self._backend.dialect.format_identifier(table), schema)

    async def index_list(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA index_list(table)`` rows."""
        return await self.execute("index_list", self._backend.dialect.format_identifier(table), schema)

    async def index_info(self, index: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA index_info(index)`` rows."""
        return await self.execute("index_info", self._backend.dialect.format_identifier(index), schema)

    async def index_xinfo(self, index: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA index_xinfo(index)`` rows (SQLite 3.9+)."""
        return await self.execute("index_xinfo", self._backend.dialect.format_identifier(index), schema)

    async def foreign_key_list(self, table: str, schema: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return raw ``PRAGMA foreign_key_list(table)`` rows."""
        return await self.execute("foreign_key_list", self._backend.dialect.format_identifier(table), schema)
    ...
    async def integrity_check(self, schema: Optional[str] = None) -> List[str]:
        """Run ``PRAGMA integrity_check`` and return error message strings.
    ...
    async def journal_mode(self, schema: Optional[str] = None) -> Optional[str]:
        """Return the current journal mode."""
