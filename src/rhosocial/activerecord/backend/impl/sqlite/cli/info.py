# src/rhosocial/activerecord/backend/impl/sqlite/cli/info.py
"""info subcommand - Display SQLite environment information.

The info command shows information derived entirely from the Python sqlite3
module version, independent of any database file, so it does not need
--db-file or other connection arguments.
"""
...
from .connection import create_backend_from_memory
...
def handle(args):
    ...
    info uses an in-memory database; displayed information is determined
    entirely by the Python sqlite3 module version.
    """
    backend = create_backend_from_memory()
    ...
    # Add connection info (always memory for info)
    info["database"]["database_type"] = "memory"
    info["database"]["database_path"] = ":memory:"
    ...
    if output_format == "json" or not RICH_AVAILABLE:
        ...
        _display_info_rich(info_legacy, args.verbose, sqlite_version, False, ":memory:")
...
def _check_protocol_support(dialect: Any, protocol_class: type) -> Dict[str, Any]:
    """Check all support methods for a protocol against the dialect."""
    results = {}
    methods = _get_protocol_support_methods(protocol_class)
...
def _build_protocol_group_info(dialect: Any, protocols: list, verbose: int) -> Dict[str, Any]:
    ...
    for protocol in protocols:
        ...
        support_methods = _check_protocol_support(dialect, protocol)
