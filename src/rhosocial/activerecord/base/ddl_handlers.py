...
``DDLFieldAnnotationHandler``
    ...
    - ``__ddl_field_sql_types__``   ``{field_name: UseSqlType}``
    - ``__ddl_field_indexes__``     ``{field_name: [IndexDefinition, ...]}``
    - ``__ddl_field_constraints__`` ``{field_name: [ColumnConstraint, ...]}``

``DDLModelAnnotationHandler``
    Reads model-level ``__table_options__``, ``__indexes__``, ``__constraints__``
    ...
    Field-level ``UseIndex`` entries (from ``__ddl_field_indexes__``) are
...
class DDLFieldAnnotationHandler:
    ...
    def handle(new_class: type) -> None:
        for attr in (
            "__ddl_field_sql_types__",
            "__ddl_field_indexes__",
            "__ddl_field_constraints__",
        ):
        ...
        for field_name, field_type in hints.items():
            if _skip_classvar(field_type):
                continue
            col_name = (
                new_class._get_column_name(field_name)
                if hasattr(new_class, "_get_column_name")
                else field_name
            )
        ...
        new_class.__ddl_field_sql_types__ = sql_types
        new_class.__ddl_field_indexes__ = indexes
        new_class.__ddl_field_constraints__ = constraints


class DDLModelAnnotationHandler:
    """Read ``__table_options__``, ``__indexes__``, ``__constraints__``
    class variables and write the final, merged collections to the class.

    Merges field-level ``UseIndex`` entries (``__ddl_field_indexes__``) into
    ...
    def handle(new_class: type) -> None:
        ...
        constraint_list: List[TableConstraint] = list(
            getattr(new_class, "__constraints__", None) or []
        )
        new_class.__ddl_constraints__ = constraint_list

        raw_indexes: Optional[List[Any]] = (
            getattr(new_class, "__indexes__", None) or []
        )
        ...
        field_dict: Dict[str, List[IndexDefinition]] = getattr(
            new_class, "__ddl_field_indexes__", {}
        )
    ...
    def _validate_index(entry: Any) -> Optional[IndexDefinition]:
        ...
        if isinstance(entry, dict):
            try:
                ...
                raise ValueError(
                    f"Invalid IndexDefinition dict in '__indexes__': {exc}"
                ) from exc
        raise TypeError(
            f"__indexes__ entries must be IndexDefinition or dict, "
