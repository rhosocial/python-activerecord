...
> **Declaring partitions.** Partitions are declared by constructing
> backend-specific expression classes and passing them to
> `CreateTableExpression(partition=...)`. The example below shows the
