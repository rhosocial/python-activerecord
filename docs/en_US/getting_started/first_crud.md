...
> 💡 **AI Prompt:** "What is a virtual environment and why should I use one?"
...
c: ClassVar[FieldProxy] = FieldProxy()
```
- **You define this**: The field proxy is not built-in—every model must define it to enable type-safe queries
...
> 💡 **AI Prompt:** "Explain what FieldProxy does and why it's needed for type-safe queries."
...
**Output:**
```
✅ Added task: Learn rhosocial-activerecord (ID: 1)
✅ Added task: Build a Todo app (ID: 2)
✅ Added task: Review the code (ID: 3)
```
...
> 💡 **AI Prompt:** "What's the difference between creating an instance and calling save()?"
...
> 💡 **AI Prompt:** "Show me different ways to filter and sort tasks in this Todo app."
...
def update_task_title(task_id: int, new_title: str) -> bool:
    ...
    task.save()
    print(f"✏️ Updated task {task_id}: {new_title}")
    return True
...
> 💡 **AI Prompt:** "How does rhosocial-activerecord know whether to INSERT or UPDATE?"
...
✅ **Create**: `Model(**data).save()` → INSERT  
✅ **Read**: `Model.query().where(...).all()` → SELECT  
✅ **Update**: Modify attributes, then `.save()` → UPDATE  
...
- [Configuration](configuration.md) — Database setup details
- [Querying](../querying/README.md) — Advanced query features
- [Common Errors](troubleshooting.md) — If you run into issues
