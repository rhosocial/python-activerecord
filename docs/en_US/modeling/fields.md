...
`UseSqlType` can declare **multiple** types at once. At DDL generation time, the framework picks the **first** type the current dialect can render; if none matches, it falls back to auto-inference (`suggest_column_type`), and raises if that yields nothing either.
