...
| Timestamp | `created_at`, `updated_at` | Auto-generated by DefaultTimestampMixin |
...
class Post(ActiveRecord):
    ...
    # Inherit DefaultTimestampMixin for automatic created_at and updated_at
```
...
class Post(UUIDMixin, DefaultTimestampMixin, DefaultSoftDeleteMixin, ActiveRecord):
    ...
    - created_at: Creation time (from DefaultTimestampMixin)
    - updated_at: Update time (from DefaultTimestampMixin)
    - deleted_at: Soft delete marker (from DefaultSoftDeleteMixin)
    """
    title: str
    content: str
```
...
| **Consistent with project conventions** | No | Yes (`DefaultTimestampMixin`, `DefaultSoftDeleteMixin`, etc.) |
