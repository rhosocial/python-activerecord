...
Since rhosocial-activerecord models inherit directly from Pydantic V2's `BaseModel`, you can leverage Pydantic's powerful serialization capabilities out of the box, converting between models, dictionaries, and JSON in a type-safe way.
...
Runnable examples live in `docs/examples/chapter_11_serialization/`:

| File | Description |
|------|-------------|
| `01_basic_serialization.py` | Basic serialization: `model_dump()`, `model_dump_json()`, field type handling |
| `02_field_filtering.py` | Field filtering: `exclude`, `include`, nested filtering, context awareness |
| `03_related_data.py` | Related data: serializing relations, `computed_field`, nested patterns |
