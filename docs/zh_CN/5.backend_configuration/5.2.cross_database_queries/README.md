...
本节介绍如何在Python ActiveRecord中同时使用多个数据库，包括连接不同的数据库系统、集成异构数据源、在数据库之间同步数据以及处理跨多个数据库的事务。
...
Python ActiveRecord抽象了许多特定于数据库的差异，允许您编写适用于不同数据库系统的查询：
...
当使用多个数据库时，您可能需要在它们之间同步数据。Python ActiveRecord提供了几种数据同步方法。
...
处理跨多个数据库的事务是具有挑战性的，因为大多数数据库系统本身不支持分布式事务。Python ActiveRecord提供了几种策略来管理跨数据库操作。
