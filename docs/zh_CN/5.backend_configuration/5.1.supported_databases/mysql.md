...
Python ActiveRecord 为 MySQL 数据库系统提供了全面的支持。本文档涵盖了在使用 Python ActiveRecord 与 MySQL 时的特定功能、配置选项和注意事项。

> **重要提示**：MySQL 后端正在作为单独的代码包开发中，将在未来发布。本文档作为即将推出的功能的参考提供。

## 概述

MySQL 是世界上最流行的开源关系型数据库管理系统之一。Python ActiveRecord 的 MySQL 后端提供了一个一致的接口，同时尊重 MySQL 的独特特性。
...
要将 MySQL 与 Python ActiveRecord 一起使用，您需要使用 MySQL 后端配置您的模型：
...
Python ActiveRecord 将 Python 数据类型映射到 MySQL 数据类型，以下是主要的映射关系：
...
MySQL 支持多种存储引擎，每种都有其特点和用例。Python ActiveRecord 允许您在表级别指定存储引擎：
...
MySQL 支持事务（使用 InnoDB 引擎），Python ActiveRecord 提供了简单的事务管理接口：
...
Python ActiveRecord 支持 MySQL 的锁定功能，用于处理并发访问：
...
MySQL 5.7+ 提供了原生 JSON 支持，Python ActiveRecord 允许您使用这些功能：
...
MySQL 提供全文搜索功能，Python ActiveRecord 支持这一特性：
...
Python ActiveRecord 的 MySQL 后端提供了一个强大而灵活的接口，用于与 MySQL 数据库交互。通过利用 MySQL 的特定功能，同时保持 ActiveRecord 的简洁 API，您可以构建高效且可维护的数据库驱动应用程序。