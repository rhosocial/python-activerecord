...
4.  **测试友好**: 与需要数据库连接来测试 SQL 生成的系统不同，我们的虚拟后端允许在没有任何外部依赖的情况下完成 SQL 生成逻辑的测试。
5.  **即时验证**: 任何表达式都可以立即调用 `to_sql()` 来验证输出，无需通过复杂的查询编译管道执行。
6.  **不支持 Mock**: 与某些测试框架不同，它**不支持**预设返回值（Mocking responses）。
