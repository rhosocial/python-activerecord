...
由于 rhosocial-activerecord 模型直接继承自 Pydantic V2 的 `BaseModel`，因此你可以直接使用 Pydantic 强大的序列化功能，在模型与字典、JSON 之间进行类型安全的转换。
...
完整的可运行示例位于 `docs/examples/chapter_11_serialization/`：

| 文件 | 说明 |
|------|------|
| `01_basic_serialization.py` | 基本序列化：`model_dump()`、`model_dump_json()`、字段类型处理 |
| `02_field_filtering.py` | 字段过滤：`exclude`、`include`、嵌套过滤、上下文感知 |
| `03_related_data.py` | 关联数据：序列化关系、`computed_field`、嵌套模式 |