...
## 1. 定义你的数据：所见即所得
一切从定义模型开始。在 `rhosocial-activerecord` 中，你的模型 **本质上就是** Pydantic V2 模型。

> 💡 **什么是 Pydantic？** Pydantic 是一个使用 Python 类型提示进行数据验证的库。它确保数据在运行时符合指定的类型和约束。你可以把它想象成数据的"类型安全卫士"——如果你说一个字段应该是 18 到 100 之间的整数，Pydantic 会立即拒绝任何不符合这些条件的数据，甚至在数据到达数据库之前。
>
> 这意味着你不需要学习一套新的验证规则——你可以使用你已经熟悉的 Pydantic 模式（或者可以快速学习）。
