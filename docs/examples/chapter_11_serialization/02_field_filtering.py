...
def main():
    ...
    filtered_profile = profile.model_dump(exclude={"address": {"zip"}, "settings": {"notifications"}})
    print_json(filtered_profile, "Filtered nested fields")

    # Include only specific nested fields
    minimal_address = profile.model_dump(include={"id": True, "user_id": True, "address": {"city": True}})
    ...
    def serialize_for_api(user: User) -> dict:
        ...
        return user.model_dump(include={"id", "username", "email", "created_at"})
