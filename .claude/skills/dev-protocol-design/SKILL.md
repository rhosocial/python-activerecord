...
11. **The backend never serves ActiveRecord** - ActiveRecord is the backend's *user*; the backend knows nothing about ActiveRecord and describes only generic SQL / concrete database capabilities. ActiveRecord must adapt to the backend. When a feature requires backend cooperation, negotiate it as a **protocol/interface contract** on the backend's generic terms — never change backend design to fit ActiveRecord.
12. **Never make the dialect interpret SQL text** - forbidded: ActiveRecord-serving dialect helpers (`ddl_inline`, `inline_params`, parameter-inlining / SQL-rewriting machinery) and any inference from **string characteristics** (regex matches, prefix/suffix checks, statement-type sniffing from leading keywords, etc.).
13. **`to_sql()` is terminal and parameterless (IRON RULE)** - `to_sql()` is the ONLY method on expressions and **accepts NO parameters whatsoever**: no `to_sql(inline_literals=...)`, no keyword arguments, no rendering options. Its signature is fixed by `ToSQLProtocol` as `to_sql(self) -> SQLQueryAndParams`. All construction inputs are collected at instantiation time.
14. **`format_*()` formatting functions accept expression instances only (IRON RULE)** - a formatting function receives the expression/predicate it must render (and the dialect), and nothing else: no `format_*(..., inline_literals=...)`, no rendering flags, no per-call formatting options. **Every formatting choice is a construction-time (factory) choice**: if output format must differ (e.g. inline literal inside DDL), the expression class captures it as a constructor parameter at instantiation, and `to_sql()` / `format_*()` are pure reads of already-collected state. **`render*()` methods, `to_sql_inline()` / `to_sql_inline_literals()` and any other "variant rendering" entry points are absolutely forbidden.** If a render-time choice appears necessary, the expression was constructed wrong.
15. **No private functions in backend protocol/implementation — everything is transparent** - all behaviour is reachable through the public surface (`to_sql()` on expressions; `format_*()`/`supports_*()` on the dialect; expression/statement classes; type adapters). **If you feel the urge to add a private helper function, you are doing something wrong — stop**: it either re-implements/pre-empts public protocol behaviour (a protocol leak that bypasses the negotiated, backend-overridable interface) or hides a design gap behind an unexposed shortcut; the fix belongs in the public protocol or in construction.
16. **Backend protocol changes require extreme restraint, deep deliberation, broad research** - the protocol is a shared contract implicated across every backend; changing any part is a large cross-cutting project. Before touching it: broadly research every backend's implementation/tests and validate against real servers; deliberate (minimal, generic, composes with existing contracts); then negotiate and implement across the generic layer and all backends together with identical behaviour. Hasty protocol changes (invented render helpers, parameterized `to_sql()`/`format_*()`) commit every backend to unsurveyed behaviour, fracture the contract, and force costly retrofits — exactly what repeatedly poisoned the abandoned DDL-derivation work (`ddl_inline`, `inline_params`, string-sniffing).
17. **Generic vs concrete expression placement** - features with a broad base across databases are lifted to the generic (core) layer (protocol + generic mixin + `supports_*()`); features without a broad base (e.g. table partitioning) are implemented per-backend and **prefixed with the backend name** (`MySQLRangePartition`, `PostgresRangePartition`, …). Goal: the generic layer implements as much as possible to minimise backend work. When a generic implementation does not fit a concrete backend, that backend overrides the affected method(s) — the correctness escape hatch, not a reason to avoid lifting broadly-shared features to the core.
